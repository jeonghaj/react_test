...
function Cafe() {
    ...
    */
    ...
    const move = (pageNum=1)=>{
        // 검색 조건에 맞는 query 문자열을 얻어내기
        const query = new URLSearchParams(searchState).toString()
        navigate(`/cafes?pageNum=${pageNum}&${query}`)
    }

    //검색 조건(condition)을 변경하거나 검색어(keyword)를 입력하면 호출되는 함수
    ...
    return (
        <>
            ...
            <Pagination className="mt-3">
                <Pagination.Item onClick={()=>{move(pageInfo.pageNum - 1)}} disabled={pageInfo.startPageNum === 1}>&laquo;</Pagination.Item>
                {
                    pageArray.map(item=>(
                ...
                <Pagination.Item onClick={()=>{move(pageInfo.pageNum + 1)}} disabled={pageInfo.endPageNum >= pageInfo.totalPageCount}>&raquo;</Pagination.Item>            
            </Pagination>
            ...
            <input onChange={handleSearchChange} value={searchState.keyword} type="text" placeholder="검색어 입력" name="keyword"/>

            <button onClick={()=>{move()}}>Search</button>
