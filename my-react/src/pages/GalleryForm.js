...
function GalleryForm() {
    ...
    const handleUpload = () => {
        // 입력한 제목과 이미지 파일을 FormData 객체에 담기
        const formData = new FormData()
        // caption 이라는 키값으로 제목 담기
        formData.append("caption", captionInput.current.value)
        // 반복문 돌면서 선택한 파일을 모두 images 라는 키값으로 담아서 ( 파일이 여러개일 경우 )
        const files = imageInput.current.files
        for (let i = 0; i < files.length; i++) {
            formData.append("images", imageInput.current.files[i])
        }
    ...
    return (
        <>
            <Form.Group className="mb-3" controlId="caption">
                ...
                <Form.Control type="text" ref={captionInput} name="caption" placeholder="설명 입력..." />
            </Form.Group>
            ...
            <a href="about:blank" onClick={(e) => {
                e.preventDefault() // 빈페이지를 띄우려고 하는 기본 동작 막기
            ...
            <Button variant="success" onClick={handleUpload}>업로드</Button>
        </>
    );
}
