...
function Gallery(props) {
    ...
    /*
        ...
        useEffect(함수, [params]) : component 활성화 시점 | params 값이 변경될때마다 호출된다.
        useEffect(함수, [state1, state2]) : component 활성화 시점 | state1 or state2 값이 변경될때 마다 호출된다.
    */
    useEffect(()=>{
        //갤러리 목록을 요청해서 읽어온다
        const pageNum = params.get("pageNum")
        refresh(pageNum)
        // params 가 변경될때 useEffect Hook 이 호출된다 
    },[params])

    return (
        <div>
            <h1>Gallery</h1>
            <Button as={Link} variant='success' to={"/gallery/new"} >Upload</Button>
            <Row>
                {
            ...
            <Pagination className='mt-3'>
                <Pagination.Item onClick={()=>{
                    //setParams() 함수를 이용해서 serch parameter 값을 변경
