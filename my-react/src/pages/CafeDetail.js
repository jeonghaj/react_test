        ...
        axios[method](action, formData)
            .then(res => {
                ...
                */
                ...
                //이 댓글을 commetIndex 에 끼워 넣기
                // commentIndex = 원글인 경우 : 0 / 대댓글인 경우
                ...
                <ul>
                    {
                        commentList.map((item, index) => (
                            <li key={item.num}
                                ref={item.ref}
                                // indent = 들여쓰기
                                // comment_group 과 같으면 원글의 댓글 다르면 대댓글
                                className={cx({ indent: item.num !== item.comment_group })}>
                                {/* 화살표 이미지 // 대댓글인 경우 inline 원 댓글인 경우 none */}
                                <svg style={{
                                ...
                                <form action={`/cafes/${num}/comments`}
                                    ...
                                    <button type="submit" onClick={() => {
                                        // item.ref.current = 폼이 속해있는 li 요소의 참조값
                                        // 그 li 요소 안에서 폼을 찾는다.
